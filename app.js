/* Bingo Beats V112 SPOTIFY ITEMS ENDPOINT FIX */
...
(function(){
  ...
  async function bbImportSelectedPlaylist(){
    try{
      ...
      setPlaylistStatus(`Playlist ophalen v112: ${playlist.name}...`, null);

      // V112: Spotify accepteert /playlists/{id}/tracks niet meer betrouwbaar.
      // Gebruik de officiële Playlist Items endpoint: /playlists/{id}/items.
      // Let op: max limit is 50 bij dit endpoint.
      const cleanId = encodeURIComponent(playlistId);
      let items = await fetchAllSpotifyPages(`https://api.spotify.com/v1/playlists/${cleanId}/items?limit=50&additional_types=track`);
      const seen = new Set();
      const imported = [];
