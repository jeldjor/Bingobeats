/* Bingo Beats V109 GITHUB CACHE + SPOTIFY DEBUG */
...
(function(){
  ...
  async function bbImportSelectedPlaylist(){
    try{
      ...
      setPlaylistStatus(`Playlist ophalen v110: ${playlist.name}...`, null);

      // V108: importeer zonder market=from_token. Sommige accounts/playlists geven daarop 403.
      // We gebruiken eerst de simpele playlist-tracks endpoint. Als die faalt, proberen we nog de href uit /me/playlists.
      const cleanId = encodeURIComponent(playlistId);
      let items = [];
      let firstErr = null;
      try{
        items = await fetchAllSpotifyPages(playlist.tracks?.href || `https://api.spotify.com/v1/playlists/${cleanId}/tracks?limit=100`);
      }catch(e){
        firstErr = e;
        if(playlist.tracks?.href){
          items = await fetchAllSpotifyPages(`https://api.spotify.com/v1/playlists/${cleanId}/tracks?limit=100`);
        }else{
          throw e;
        }
      }
      ...
      if(/scope|permission|forbidden|insufficient|rechten|403/i.test(msg)){
        help += ' — Stuur deze exacte tekst naar ChatGPT. Vooral HTTP-code en endpoint zijn belangrijk.';
      }
