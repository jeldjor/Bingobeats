...
(function(){
  ...
  async function bbImportSelectedPlaylist(){
    try{
      ...
      // Gebruik de Spotify playlist tracks href uit /me/playlists.
      // Dit is betrouwbaarder dan zelf een fields-query bouwen, omdat sommige Spotify-accounts/playlisttypes
      // anders een 400/403 kunnen geven. We halen volledig op en filteren daarna in de app.
      let tracksUrl = playlist.tracks?.href || `https://api.spotify.com/v1/playlists/${encodeURIComponent(playlistId)}/tracks`;
      tracksUrl = tracksUrl.split('?')[0] + '?limit=100&market=from_token';
      const items = await fetchAllSpotifyPages(tracksUrl);
      const seen = new Set();
      const imported = [];
