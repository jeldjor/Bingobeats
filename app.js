...
/* ==================================================
   V161 - stabiele schermwissel + responsive lobby
   - Geen flits van oud scherm: lobby wordt direct gerenderd, niet via setTimeout.
   - Lobby/score/live lijsten blijven compact en scrollbaar.
   - Spelersvenster én eigen bingokaart blijven tegelijk zichtbaar.
   ================================================== */
(function(){
  const q = id => document.getElementById(id);
  const html = v => String(v ?? '').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));
  function playerName(p){ return String(p?.name || 'Speler').replace(/^🎤\s*/,'').trim() || 'Speler'; }
  function playerIsHost(p){ return !!p?.isHost || /^🎤/.test(String(p?.name || '')); }
  function colHex(c){
    return ({yellow:'#FFCC33',pink:'#00D4C7',purple:'#FF8A1F',blue:'#7ED957',green:'#FF5A5F',free:'#151515'}[c] || '#222');
  }
  function playersSorted(room){
    return Object.entries(room?.players || {}).sort((a,b)=>{
      const ar = a[1]?.ready ? 1 : 0;
      const br = b[1]?.ready ? 1 : 0;
      if(ar !== br) return ar - br; // eerst wachten, daarna ready
      if(a[0] === currentPlayerId) return -1;
      if(b[0] === currentPlayerId) return 1;
      return playerName(a[1]).localeCompare(playerName(b[1]), 'nl');
    });
  }
  function readySummary(room){
    const list = Object.values(room?.players || {});
    return {ready:list.filter(p=>!!p.ready).length,total:list.length};
  }
  function cardGrid(card, marked){
    const arr = Array.isArray(card) ? card : [];
    const cells = arr.map((c,i)=>{
      const markedCell = !!marked?.[i] || i === 12;
      return `<div class="bbV161CardCell ${markedCell?'marked':''}" style="background:${colHex(c)}">${markedCell ? '🐵' : ''}</div>`;
    }).join('');
    return `<div class="bbV161MiniCard">${cells}</div>`;
  }
  function playersBox(room){
    const rc = readySummary(room);
    const rows = playersSorted(room).map(([pid,p])=>{
      const own = pid === currentPlayerId;
      return `<button type="button" class="bbV161Player ${p.ready?'ready':'wait'} ${own?'own':''}" onclick="bbV161ShowCard('${html(pid)}')">
        <span class="bbV161PName">${playerIsHost(p)?'🎤':'👤'} ${html(playerName(p))}${own?' <em>jij</em>':''}</span>
        <span class="bbV161PIcon">${p.ready?'✅':'⏳'}</span>
      </button>`;
    }).join('');
    return `<section class="bbV161PlayersBox">
      <div class="bbV161ReadyTop">👥 ${rc.ready} / ${rc.total} READY</div>
      <div class="bbV161PlayersGrid">${rows || '<div class="bbV161Empty">Nog geen spelers.</div>'}</div>
      <div class="bbV161Hint">Klik op de speler voor de bingokaart.</div>
    </section>`;
  }
  function renderLobby161(root, room, title, sub){
    const me = room?.players?.[currentPlayerId] || {};
    const isReady = !!me.ready;
    document.body.classList.add('bbV161Fit');
    root.className = 'compactDashboard bbV161LobbyStage';
    root.innerHTML = `
      <section class="bbV161Hero"><h2>${html(title || 'Wachten op de host')}</h2><p>${html(sub || 'Iedereen klaar? Dan kan de ronde starten.')}</p></section>
      ${playersBox(room)}
      <section class="bbV161OwnCard"><div class="bbV161OwnTitle">Jouw bingokaart</div>${cardGrid(me.card, me.marked || {})}</section>
      <div class="bbV161Bottom">${isReady ? '<button class="bbV161Ready isReady" disabled>READY ✓</button>' : '<button class="bbV161Ready" onclick="bbV161Ready()">READY</button>'}</div>
    `;
  }
  function renderLive161(root, room, r){
    const answers = r?.id ? (room.answers?.[r.id] || {}) : {};
    const rows = playersSorted(room).map(([pid,p])=>{
      const has = answers[pid] && typeof answers[pid].answer !== 'undefined';
      const text = has ? (String(answers[pid].answer).trim() || 'Leeg antwoord') : 'Geen antwoord';
      return `<div class="bbV161AnswerRow ${has?'sent':'wait'}">
        <span class="bbV161AnswerName">${playerIsHost(p)?'🎤':'👤'} ${html(playerName(p))}${pid===currentPlayerId?' <em>jij</em>':''}</span>
        <span class="bbV161AnswerText">${html(text)}</span>
      </div>`;
    }).join('');
    document.body.classList.add('bbV161Fit');
    root.className = 'compactDashboard bbV161LiveStage';
    root.innerHTML = `
      <section class="bbV161LiveHead"><h2>Live antwoorden</h2><p>Zodra iemand iets invult, verschijnt het automatisch.</p></section>
      <section class="bbV161AnswerList">${rows}</section>
    `;
  }
  window.bbV161Ready = function(){
    try{ if(currentRoomCode && currentPlayerId) db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId+'/ready').set(true); }catch(e){}
  };
  window.bbV161ShowCard = function(pid){
    const room = window.bbV161LastRoom || window.bbV160LastRoom || {};
    const p = room.players?.[pid];
    if(!p) return;
    let modal = q('bbV161CardModal');
    if(!modal){
      modal = document.createElement('div');
      modal.id = 'bbV161CardModal';
      modal.className = 'bbV161CardModal hidden';
      document.body.appendChild(modal);
    }
    modal.innerHTML = `<div class="bbV161CardPop">
      <button type="button" class="bbV161Close" onclick="bbV161CloseCard()">×</button>
      <h2>${playerIsHost(p)?'🎤':'👤'} ${html(playerName(p))}</h2>
      ${cardGrid(p.card, p.marked || {})}
    </div>`;
    modal.classList.remove('hidden');
  };
  window.bbV161CloseCard = function(){ q('bbV161CardModal')?.classList.add('hidden'); };

  const oldRender = (typeof renderCompactDashboard === 'function') ? renderCompactDashboard : null;
  if(oldRender){
    renderCompactDashboard = function(room, r){
      window.bbV161LastRoom = room || {};
      window.bbV160LastRoom = room || {};
      const root = q('screenDashboard');
      if(!root) return oldRender.apply(this, arguments);

      const status = r?.status || '';
      const hasRound = !!r?.id;
      const ownSubmitted = hasRound && !!room?.answers?.[r.id]?.[currentPlayerId];

      // Geen actieve ronde: compacte lobby direct tonen.
      if(!hasRound){
        renderLobby161(root, room, 'Wachten op de host', 'Iedereen klaar? Dan kan de ronde starten.');
        return;
      }

      // Tijdens antwoorden na eigen inzending: compact live-overzicht.
      if(status === 'answering' && ownSubmitted){
        renderLive161(root, room, r);
        return;
      }

      // BELANGRIJK: einde ronde niet overslaan.
      // Eerst laat de bestaande flow netjes zien:
      // 1. juiste antwoord -> 2. scoreboard -> 3. bingokaart kiezen -> 4. lobby.
      // Alleen als de bestaande flow echt in de lobby staat, vervangen we die lobby door de compacte V161-lobby.
      if(['locked','review','judged'].includes(status)){
        document.body.classList.remove('bbV161Fit');
        const result = oldRender.apply(this, arguments);
        setTimeout(()=>{
          try{
            const isLobby = root.className.includes('bbV144Lobby') ||
                            root.className.includes('bbV143Lobby') ||
                            root.querySelector('.bbV144ReadyList,.bbV143ReadyRow,.stageLobbyV143,.stageLobby');
            if(isLobby){
              renderLobby161(root, room, 'Klaar voor de volgende?', 'Klik op READY voor de volgende ronde.');
            }
          }catch(e){}
        },0);
        return result;
  ...
  // Host spelerslijst ook dezelfde compacte stijl.
  if(typeof renderHostPlayers === 'function'){
    renderHostPlayers = function(room){
      const box = q('hostPlayers');
      if(!box) return;
      const rc = readySummary(room);
      const rows = playersSorted(room).map(([pid,p])=>`<button type="button" class="bbV161HostP" onclick="bbV161ShowCard('${html(pid)}')"><span>${playerIsHost(p)?'🎤':'👤'} ${html(playerName(p))}</span><b>${p.ready?'✅':'⏳'}</b></button>`).join('');
      box.innerHTML = `<div class="bbV161ReadyTop host">👥 ${rc.ready} / ${rc.total} READY</div><div class="bbV161HostGrid">${rows || '<div class="small">Nog geen spelers.</div>'}</div><div class="bbV161Hint host">Klik op de speler voor de bingokaart.</div>`;
    };
  }
})();

/* =========================
   V162 - Host speelt mee: volgende ronde automatisch starten
   Als de host meespeelt en iedereen na een beoordeelde ronde READY is,
   start de volgende ronde automatisch. Alleen actief in Host-speelt-mee modus.
   ========================= */
(function(){
  let bbAutoHostPlayerRef = null;
  let bbAutoHostPlayerKey = '';
  let bbAutoHostPlayerBusy = false;

  function roomHasBingo(room, roundId){
    try{
      return Object.values(room?.bingos || {}).some(b => b && b.roundId === roundId);
  ...
  function stopHostPlayerAutoStart(){
    try{ if(bbAutoHostPlayerRef) bbAutoHostPlayerRef.off('value'); }catch(e){}
    bbAutoHostPlayerRef = null;
    bbAutoHostPlayerBusy = false;
  }

  function startHostPlayerAutoStart(roomCode){
    stopHostPlayerAutoStart();
    if(!roomCode || typeof db === 'undefined' || !db) return;
    bbAutoHostPlayerRef = db.ref('rooms/'+roomCode);
    bbAutoHostPlayerRef.on('value', snap => {
      const room = snap.val() || {};
      const r = room.currentRound || {};
      if(!document.body.classList.contains('bbHostPlayerMode')) return;
      if(!r.id || r.status !== 'judged') return;
      if(roomHasBingo(room, r.id)) return;
      if(!everyoneReady(room)) return;

      const key = roomCode + ':' + r.id;
      if(bbAutoHostPlayerKey === key || bbAutoHostPlayerBusy) return;
      bbAutoHostPlayerKey = key;
      bbAutoHostPlayerBusy = true;

      setTimeout(async () => {
        try{
          const latestSnap = await db.ref('rooms/'+roomCode).once('value');
          const latest = latestSnap.val() || {};
          const lr = latest.currentRound || {};
          if(!document.body.classList.contains('bbHostPlayerMode')) return;
          if(lr.id !== r.id || lr.status !== 'judged') return;
          if(roomHasBingo(latest, lr.id)) return;
          if(!everyoneReady(latest)) return;

          if(typeof window.bbHostPlayerStart === 'function'){
            await window.bbHostPlayerStart(false);
          }else if(typeof startRound === 'function'){
            startRound();
          }
        }catch(e){
          console.error('Host-speelt-mee auto start mislukt:', e);
        }finally{
          setTimeout(()=>{ bbAutoHostPlayerBusy = false; }, 1800);
        }
      }, 900);
    });
  }

  const originalEnter = window.bbEnterHostPlayerMode;
  if(typeof originalEnter === 'function'){
    window.bbEnterHostPlayerMode = async function(){
      const result = await originalEnter.apply(this, arguments);
      const room = currentRoomCode || localStorage.hb_host_room || localStorage.hb_last_stable_room || '';
      startHostPlayerAutoStart(room);
      return result;
    };
  }

  const originalExit = window.bbHostPlayerExit;
  if(typeof originalExit === 'function'){
    window.bbHostPlayerExit = function(){
      stopHostPlayerAutoStart();
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    setTimeout(()=>{
      if(document.body.classList.contains('bbHostPlayerMode')){
        const room = currentRoomCode || localStorage.hb_host_room || localStorage.hb_last_stable_room || '';
        startHostPlayerAutoStart(room);
      }
    },1000);
...
/* =========================
   V163 - Fix Host-speelt-mee flow
   - Einde ronde slaat geen schermen meer over
   - Auto volgende ronde start robuust zodra iedereen READY is
   ========================= */
(function(){
  let busy = false;
  let lastKey = '';
  function hasBingo(room, roundId){
    try{ return Object.values(room?.bingos || {}).some(b => b && b.roundId === roundId); }catch(e){ return false; }
  }
  ...
  async function checkAutoStart(){
    try{
      ...
      const roomCode = currentRoomCode || localStorage.hb_host_room || localStorage.hb_last_stable_room || '';
      if(!roomCode) return;
      const snap = await db.ref('rooms/'+roomCode).once('value');
      const room = snap.val() || {};
      const r = room.currentRound || {};
      if(!r.id || r.status !== 'judged') return;
      if(hasBingo(room, r.id)) return;
      if(!allReady(room)) return;
      const key = roomCode + ':' + r.id;
      if(busy || lastKey === key) return;
      busy = true;
      lastKey = key;
      setTimeout(async()=>{
        try{
          ...
          const r2 = room2.currentRound || {};
          if(!document.body.classList.contains('bbHostPlayerMode')) return;
          if(r2.id !== r.id || r2.status !== 'judged') return;
          if(hasBingo(room2, r2.id)) return;
          if(!allReady(room2)) return;
          if(typeof window.bbHostPlayerStart === 'function'){
            window.bbHostPlayerStart(false);
          }else if(typeof startRound === 'function'){
            startRound();
          }
        }catch(e){ console.error('V163 auto-start fout:', e); }
        finally{ setTimeout(()=>{ busy = false; }, 2200); }
      },650);
    }catch(e){ console.error('V163 checkAutoStart fout:', e); busy = false; }
  }
  const old161Ready = window.bbV161Ready;
  window.bbV161Ready = function(){
    if(typeof old161Ready === 'function') old161Ready.apply(this, arguments);
    setTimeout(checkAutoStart,250);
    setTimeout(checkAutoStart,1000);
  };
  const old144Ready = window.bbV144Ready;
  window.bbV144Ready = function(){
    if(typeof old144Ready === 'function') old144Ready.apply(this, arguments);
    setTimeout(checkAutoStart,250);
    setTimeout(checkAutoStart,1000);
  };
  const oldHostReady = window.bbHostPlayerReady;
  window.bbHostPlayerReady = function(){
    if(typeof oldHostReady === 'function') oldHostReady.apply(this, arguments);
    setTimeout(checkAutoStart,250);
    setTimeout(checkAutoStart,1000);
  };
  setInterval(checkAutoStart,1800);
})();
