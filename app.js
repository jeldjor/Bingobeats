  ...
  window.bbHostPlayerOpenJury = function(){
    ...
    db.ref('rooms/'+currentRoomCode).once('value').then(s=>{
      ...
      const rows = Object.entries(room.players||{}).map(([pid,p])=>{
        ...
        return `<div class="bbHostJuryRow bbHostJuryRowCompact ${st===true?'good':st===false?'bad':'wait'}">
          <div class="bbHostJuryName"><strong>${E(p.name||'Speler')}</strong></div>
          <div class="bbHostJuryAnswer">${E(a)}</div>
          <div class="bbHostJuryBtns">
            <button type="button" class="bbHostJuryIconBtn good" title="Goed" aria-label="Goed" onclick="bbHostPlayerSetJudge('${E(pid)}',true)">✅</button>
            <button type="button" class="bbHostJuryIconBtn bad" title="Fout" aria-label="Fout" onclick="bbHostPlayerSetJudge('${E(pid)}',false)">❌</button>
          </div>
        </div>`;
      }).join('');
