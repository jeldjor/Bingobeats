...
(function(){
  ...
  function pickCardScreen(root,room,r){
    ...
    const cells = Array.from({length:25},(_,i)=>{
      ...
      const isMarked = !!marked[i] || key==='free';
      const canPick = key===r.colorKey && key!=='free' && !marked[i];
      const content = isMarked ? '<span class="bbV144Mark">BB</span>' : `<span class="bbV144Emoji">${EMO[key]||''}</span>`;
  ...
  function liveAnswerScreen(root,room,r){
    ...
    const t = answerTime(room,r,currentPlayerId);
    stage(root,'bbV144Live',`
      <div class="bbV144SentCard"><div>✓</div><span>Antwoord verzonden</span><strong>${E(String(own?.answer||'').trim()||'Leeg antwoord')}</strong>${t?`<small>⚡ Ingevuld na ${E(t)}</small>`:''}</div>
      <div class="bbV144TopTitle"><span>📨</span><h2>Live antwoorden</h2></div>
      <p class="bbV144Sub">Nog niemand ziet wat goed of fout is.</p>
      <div class="bbV144ScoreList live">${rows}</div>
    `);
  }
  if(typeof renderCompactDashboard === 'function'){
    const prev = renderCompactDashboard;
