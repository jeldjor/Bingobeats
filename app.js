...
(function(){
  ...
  async function bbImportSelectedPlaylist(){
    try{
      ...
      const fields = encodeURIComponent('items(track(id,name,uri,artists(name),album(name,release_date),duration_ms,is_local)),next');
      const items = await fetchAllSpotifyPages(`https://api.spotify.com/v1/playlists/${encodeURIComponent(playlistId)}/tracks?limit=100&fields=${fields}`);
      const seen = new Set();
      const imported = [];
      ...
      console.error(e);
      setPlaylistStatus('Playlist importeren mislukt. Controleer of je opnieuw bent ingelogd met de juiste Spotify-rechten.', false);
    }
  }
