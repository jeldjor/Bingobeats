...
(function(){
  ...
  function injectSettings(){
    ...
    block.innerHTML = `
      ...
      <p class="small">Laat de host ook als speler meedoen in een rustige lobby.</p>
  ...
  function canShowJury(room,r){
    if(!r || !r.id) return false;
    if(!['locked','review','judged'].includes(r.status)) return false;
    const me = (room.players||{})[currentPlayerId] || {};
    const st = room.correct?.[r.id]?.[currentPlayerId];
    const picked = me.lastPickedRound === r.id;
    // Pas nadat de host zijn eigen route heeft gedaan: bij fout of na gekozen bingovakje.
    return st === false || picked || me.ready;
  }
  ...
  function enhanceHostLobby(room,r){
    ...
    const root = q('screenDashboard');
    if(!root || q('bbHostPlayerDock')) return;
    const isLobby = !!root.querySelector('.bbV144Lobby,.stageLobbyV143,.stageLobbyV142,.stageLobbyV141,.stageLobbyV140,.stageLobbyV139') || !r?.id;
    const dock = document.createElement('div');
    dock.id = 'bbHostPlayerDock';
    dock.className = 'bbHostPlayerDock';
    const jury = canShowJury(room,r) ? `<button type="button" class="bbHostMiniBtn jury" onclick="bbHostPlayerOpenJury()">⚖️ Jury</button>` : '';
    const roundButtons = isLobby ? `<button type="button" class="bbHostMiniBtn test" onclick="bbHostPlayerStart(true)">🧪 Testronde</button><button type="button" class="bbHostMiniBtn start" onclick="bbHostPlayerStart(false)">▶️ Start ronde</button>` : '';
    dock.innerHTML = `
      <button type="button" class="bbHostExitBtn" title="Terug naar hostscherm" onclick="bbHostPlayerExit()">🚪</button>
      <div class="bbHostMiniTitle">🎤 Host speelt mee</div>
      <div class="bbHostMiniActions">
        ${jury}
        ${roundButtons}
      </div>
    `;
