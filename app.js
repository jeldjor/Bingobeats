...
(function(){
  ...
  function canShowJury(room,r,root){
    ...
    // Jury/overrule alleen zichtbaar in de lobby na de huidige ronde.
    if(!isHostPlayerLobby(root || q('screenDashboard'), r)) return false;
    if(!['locked','review','judged'].includes(r.status)) return false;
    const me = (room.players||{})[currentPlayerId] || {};
    const st = room.correct?.[r.id]?.[currentPlayerId];
    const picked = me.lastPickedRound === r.id;
    // Pas nadat de host zijn eigen antwoord/kaart heeft afgerond.
