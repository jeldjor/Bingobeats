...
(function(){
  ...
  function startHostPlayerListener(room){
    ...
    hostPlayerRoomRef.on('value',s=>{
      ...
      try{ enhanceHostLobby(data,r); }catch(e){}
      requestAnimationFrame(()=>{ try{ enhanceHostLobby(data,r); }catch(e){} });
    });
    listenBingo(room);
  }
...
(function(){
  ...
  if(prevRender){
    renderCompactDashboard = function(room,r){
      ...
      // Vervang lobby-vensters direct in dezelfde render-tick.
      // Hierdoor ziet de speler niet eerst heel kort een oud/tussen-scherm.
...
/* V164 - centrale anti-flits fix.
   Houdt de huidige flow gelijk, maar voorkomt dat tussen-schermen zichtbaar worden
   tijdens snelle Firebase renders en dashboardwissels. */
(function(){
  const q = id => document.getElementById(id);
  function releaseSwap(){
    requestAnimationFrame(()=>requestAnimationFrame(()=>document.body.classList.remove('bbNoFlickerSwap')));
  }
  if(typeof renderCompactDashboard === 'function' && !window.__bbNoFlickerRenderWrapV164){
    window.__bbNoFlickerRenderWrapV164 = true;
    const prev = renderCompactDashboard;
    renderCompactDashboard = function(room,r){
      const root = q('screenDashboard');
      if(root && !root.classList.contains('hidden')) document.body.classList.add('bbNoFlickerSwap');
      try { return prev.apply(this, arguments); }
      finally { releaseSwap(); }
    };
  }
  if(typeof showDashboard === 'function' && !window.__bbNoFlickerShowDashV164){
    window.__bbNoFlickerShowDashV164 = true;
    const prevShow = showDashboard;
    showDashboard = function(){
      document.body.classList.add('bbNoFlickerSwap');
      try { return prevShow.apply(this, arguments); }
      finally { releaseSwap(); }
    };
  }
})();
