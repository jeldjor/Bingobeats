...
/* =========================
   V159 - Compacte lobby's + scoreboards + live antwoorden
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  const E = s => (typeof esc === 'function' ? esc(String(s ?? '')) : String(s ?? '').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])));
  const cleanName = name => String(name || 'Speler').replace(/^🎤\s*/,'').replace(/^👤\s*/,'').trim() || 'Speler';
  function isHost(pid,p){ return !!(p && (p.isHost || /^🎤/.test(String(p.name||'')) || /^host_/.test(String(pid||'')))); }
  function sortedPlayers(room){
    return Object.entries(room?.players || {}).sort((a,b)=>{
      const ah=isHost(a[0],a[1])?0:1, bh=isHost(b[0],b[1])?0:1;
      if(ah!==bh) return ah-bh;
      const ar=a[1]?.ready?1:0, br=b[1]?.ready?1:0;
      if(ar!==br) return ar-br; // eerst wacht, daarna ready
      return cleanName(a[1]?.name).localeCompare(cleanName(b[1]?.name),'nl');
    });
  }
  ...
  function lobbyMarkup(room){
    const s = readySummary(room);
    const rows = sortedPlayers(room).map(([pid,p])=>{
      const host = isHost(pid,p);
      const me = pid === currentPlayerId;
      return `<div class="bbUnifiedPlayerChip ${p.ready?'ready':'wait'} ${me?'me':''}"><span class="bbUnifiedIcon">${host?'🎤':'👤'}</span><span class="bbUnifiedName">${E(cleanName(p.name))}${me?' <em>jij</em>':''}</span><span class="bbUnifiedReady">${p.ready?'✅':'⏳'}</span></div>`;
    }).join('') || '<div class="bbUnifiedEmpty">Nog geen spelers.</div>';
    return `<div class="bbUnifiedLobbySummary">👥 ${s.ready} / ${s.total} READY</div><div class="bbUnifiedPlayerGrid">${rows}</div>`;
  }
  function scoreMarkup(room,r,withJudge){
    const ans = r?.id ? (room.answers?.[r.id] || {}) : {};
    const cor = r?.id ? (room.correct?.[r.id] || {}) : {};
    const rows = sortedPlayers(room).map(([pid,p])=>{
      const a = ans[pid] && typeof ans[pid].answer !== 'undefined' ? (String(ans[pid].answer).trim() || 'Leeg antwoord') : 'Geen antwoord';
      const st = cor[pid];
      const icon = st===true ? '✅' : st===false ? '❌' : (ans[pid] ? '⏳' : '…');
      const host = isHost(pid,p);
      const me = pid === currentPlayerId;
      const judge = withJudge ? `<span class="bbUnifiedJudge"><button type="button" data-pid="${E(pid)}" data-good="true">✅</button><button type="button" data-pid="${E(pid)}" data-good="false">❌</button></span>` : `<span class="bbUnifiedStatus">${icon}</span>`;
      return `<div class="bbUnifiedScoreRow ${st===true?'good':st===false?'bad':'wait'} ${me?'me':''}"><span class="bbUnifiedScoreName">${host?'🎤':'👤'} ${E(cleanName(p.name))}${me?' <em>jij</em>':''}</span><span class="bbUnifiedScoreAnswer">${E(a)}</span>${judge}</div>`;
    }).join('') || '<div class="bbUnifiedEmpty">Nog geen spelers.</div>';
    return `<div class="bbUnifiedScoreScroll">${rows}</div>`;
  }
  window.bbUnifiedSetJudge = function(pid,good){
    if(!currentRoomCode) return;
    db.ref('rooms/'+currentRoomCode+'/currentRound').once('value').then(s=>{
      const r=s.val()||{}; if(!r.id) throw Error('Geen actieve ronde.');
      return db.ref('rooms/'+currentRoomCode+'/correct/'+r.id+'/'+pid).set(!!good);
  ...
  // Host spelerslijst in normale hostscherm: zelfde compacte lobby-overzicht.
  if(typeof renderHostPlayers === 'function'){
    renderHostPlayers = function(room){
      const el = q('hostPlayers');
      if(!el) return;
      el.classList.add('bbUnifiedHostPlayers');
      el.innerHTML = lobbyMarkup(room);
    };
  }

  // Host scorebord: compact, scrollbaar en met kleine transparante goed/fout iconen.
  if(typeof renderHostScore === 'function'){
    renderHostScore = function(room){
      ...
      q('hostScorePanel')?.classList.remove('hidden');
      if(q('hostRoundInfo')) q('hostRoundInfo').textContent = `${r.colorEmoji||''} ${r.colorName||''} — ${r.category||''} — ${r.status||''}`;
      q('hostScoreboard').classList.add('bbUnifiedHostScoreboard');
      q('hostScoreboard').innerHTML = scoreMarkup(room,r,true);
      q('hostScoreboard').querySelectorAll('button[data-pid]').forEach(btn=>{
        btn.addEventListener('click',e=>{
          e.preventDefault();
          window.bbUnifiedSetJudge(btn.dataset.pid, btn.dataset.good === 'true');
        });
      });
    };
  }

  function enhancePlayerLobbyAndScore(room,r){
    const root = q('screenDashboard');
    if(!root) return;

    // Alle lobby-vensters vervangen door compact 2-koloms scrollvenster met ready-teller.
    const readyList = root.querySelector('.bbV144ReadyList,.bbV143ReadyList,.bbV142ReadyList,.bbV141ReadyList,.bbV140ReadyList,.bbV139ReadyList');
    if(readyList){
      readyList.classList.add('bbUnifiedReadyList');
      readyList.innerHTML = lobbyMarkup(room);
    }

    // Live antwoorden: geen groen/blauw icoon en geen tekst INGEZONDEN. Alleen naam + antwoord.
    const liveList = root.querySelector('.bbV146LiveList,.bbV144ScoreList.live');
    if(liveList){
      const ans = r?.id ? (room.answers?.[r.id] || {}) : {};
      const rows = sortedPlayers(room).map(([pid,p])=>{
        const has = ans[pid] && typeof ans[pid].answer !== 'undefined';
        const text = has ? (String(ans[pid].answer).trim() || 'Leeg antwoord') : 'Wacht op antwoord';
        const host = isHost(pid,p);
        const me = pid === currentPlayerId;
        return `<div class="bbUnifiedLiveRow ${has?'sent':'wait'} ${me?'me':''}"><span class="bbUnifiedLiveName">${host?'🎤':'👤'} ${E(cleanName(p.name))}${me?' <em>jij</em>':''}</span><span class="bbUnifiedLiveAnswer">${E(text)}</span></div>`;
      }).join('') || '<div class="bbUnifiedEmpty">Nog geen spelers.</div>';
      liveList.classList.add('bbUnifiedLiveList');
      liveList.innerHTML = rows;
    }

    // Scoreboard scherm: scrollbaar, compact en passend bij veel spelers.
    const scoreList = root.querySelector('.bbV144ScoreList:not(.live),.bbV143ScoreList:not(.live),.bbV142ScoreList:not(.live)');
    if(scoreList && r?.id){
      scoreList.classList.add('bbUnifiedResultList');
      scoreList.innerHTML = scoreMarkup(room,r,false);
    }
  }
  ...
  if(typeof renderCompactDashboard === 'function'){
    ...
    renderCompactDashboard = function(room,r){
      ...
      setTimeout(()=>enhancePlayerLobbyAndScore(room,r),0);
      setTimeout(()=>enhancePlayerLobbyAndScore(room,r),80);
      return result;
    };
  }

  // Jury/Overrule vanaf Host-speelt-mee: alleen icoontjes, net iets groter, transparant.
  if(window.bbHostPlayerOpenJury){
    const oldOpenJury = window.bbHostPlayerOpenJury;
    window.bbHostPlayerOpenJury = function(){
      oldOpenJury.apply(this,arguments);
      setTimeout(()=>{
        document.querySelectorAll('.bbHostJuryBtns button').forEach(btn=>btn.classList.add('bbUnifiedJuryIconBtn'));
      },120);
    };
  }
})();
