...
/* =========================
   V157 - Alleen Jury / Overrule echt compact
   Uitgangspunt: V155. Overige schermen blijven ongemoeid.
   ========================= */
(function(){
  function esc(v){
  ...
  function injectCompactJuryCSS(){
    if(document.getElementById('bb-v157-jury-compact-css')) return;
    const st=document.createElement('style');
    st.id='bb-v157-jury-compact-css';
    st.textContent=`
      #bbHostJuryModal.bbHostJuryModal{position:fixed!important;inset:0!important;background:rgba(0,0,0,.72)!important;z-index:99999!important;display:flex!important;align-items:center!important;justify-content:center!important;padding:8px!important;box-sizing:border-box!important;}
      #bbHostJuryModal.bbHostJuryModal.hidden{display:none!important;}
      #bbHostJuryModal .bbHostJuryCard{width:min(720px,96vw)!important;max-height:86svh!important;overflow:auto!important;border-radius:20px!important;background:#24133f!important;color:#fff!important;padding:12px!important;box-shadow:0 24px 70px rgba(0,0,0,.58)!important;position:relative!important;box-sizing:border-box!important;}
      #bbHostJuryModal .bbHostJuryClose{position:absolute!important;right:10px!important;top:10px!important;width:34px!important;height:34px!important;min-width:34px!important;min-height:34px!important;border-radius:50%!important;padding:0!important;display:flex!important;align-items:center!important;justify-content:center!important;line-height:1!important;font-size:20px!important;background:rgba(255,255,255,.12)!important;color:#fff!important;border:0!important;box-shadow:none!important;}
      #bbHostJuryModal .bbHostJuryTitle{font-size:22px!important;line-height:1.05!important;margin:0 42px 4px 0!important;font-weight:1000!important;}
      #bbHostJuryModal .bbHostJuryHelp{font-size:12px!important;line-height:1.25!important;margin:0 0 8px 0!important;opacity:.76!important;}
      #bbHostJuryModal .bbHostJuryList{display:flex!important;flex-direction:column!important;gap:5px!important;margin:0!important;padding:0!important;}
      #bbHostJuryModal .bbHostJuryRow{display:grid!important;grid-template-columns:minmax(54px,.75fr) minmax(0,1.55fr) 26px 72px!important;align-items:center!important;gap:6px!important;min-height:38px!important;padding:5px 6px!important;border-radius:11px!important;background:rgba(255,255,255,.065)!important;border:1px solid rgba(255,255,255,.12)!important;box-sizing:border-box!important;}
      #bbHostJuryModal .bbHostJuryRow.good{border-color:rgba(126,217,87,.72)!important;}
      #bbHostJuryModal .bbHostJuryRow.bad{border-color:rgba(255,90,95,.72)!important;}
      #bbHostJuryModal .bbHostJuryName{min-width:0!important;font-size:13px!important;font-weight:900!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;line-height:1.05!important;}
      #bbHostJuryModal .bbHostJuryAnswer{min-width:0!important;font-size:13px!important;font-weight:700!important;opacity:.94!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;line-height:1.05!important;}
      #bbHostJuryModal .bbHostJuryStatus{font-size:16px!important;text-align:center!important;line-height:1!important;}
      #bbHostJuryModal .bbHostJuryBtns{display:flex!important;justify-content:flex-end!important;gap:4px!important;min-width:0!important;}
      #bbHostJuryModal .bbHostJuryBtns button{width:32px!important;height:32px!important;min-width:32px!important;min-height:32px!important;padding:0!important;border-radius:10px!important;display:flex!important;align-items:center!important;justify-content:center!important;font-size:17px!important;line-height:1!important;margin:0!important;}
      #bbHostJuryModal .bbHostJuryEmpty{padding:14px!important;text-align:center!important;opacity:.8!important;font-weight:800!important;}
      @media(max-width:390px){
        #bbHostJuryModal .bbHostJuryCard{width:97vw!important;padding:10px!important;border-radius:18px!important;}
        #bbHostJuryModal .bbHostJuryRow{grid-template-columns:minmax(46px,.7fr) minmax(0,1.35fr) 22px 66px!important;gap:4px!important;min-height:36px!important;padding:4px 5px!important;}
        #bbHostJuryModal .bbHostJuryName,#bbHostJuryModal .bbHostJuryAnswer{font-size:12px!important;}
        #bbHostJuryModal .bbHostJuryBtns button{width:30px!important;height:30px!important;min-width:30px!important;min-height:30px!important;font-size:16px!important;}
      }
    `;
    document.head.appendChild(st);
  }
  injectCompactJuryCSS();
  window.bbHostPlayerOpenJury = function(){
    injectCompactJuryCSS();
    if(!currentRoomCode) return;
    db.ref('rooms/'+currentRoomCode).once('value').then(s=>{
      const room = s.val() || {}, r = room.currentRound || {};
      if(!r.id) return alert('Geen ronde om te beoordelen.');
      const ans = (room.answers && room.answers[r.id]) || {};
      const cor = (room.correct && room.correct[r.id]) || {};
      const players = room.players || {};
      const rows = Object.entries(players).map(([pid,p])=>{
        const st = cor[pid];
        const a = (ans[pid] && ans[pid].answer) ? ans[pid].answer : 'Geen antwoord';
        const statusIcon = st===true ? '✅' : st===false ? '❌' : '❔';
        const rowCls = st===true ? 'good' : st===false ? 'bad' : 'wait';
        const hostMark = p && p.isHost ? '🎤 ' : '';
        return `<div class="bbHostJuryRow ${rowCls}">
          <div class="bbHostJuryName" title="${esc(p && p.name ? p.name : 'Speler')}">${hostMark}${esc(p && p.name ? p.name : 'Speler')}</div>
          <div class="bbHostJuryAnswer" title="${esc(a)}">${esc(a)}</div>
          <div class="bbHostJuryStatus" aria-label="Status">${statusIcon}</div>
          <div class="bbHostJuryBtns">
            <button type="button" class="bbJudgeGood" title="Goed" aria-label="Goed" onclick="bbHostPlayerSetJudge('${esc(pid)}',true)">✅</button>
            <button type="button" class="bbJudgeBad" title="Fout" aria-label="Fout" onclick="bbHostPlayerSetJudge('${esc(pid)}',false)">❌</button>
          </div>
        </div>`;
      }).join('');
      let modal = document.getElementById('bbHostJuryModal');
      if(!modal){
        modal = document.createElement('div');
        modal.id = 'bbHostJuryModal';
        modal.className = 'bbHostJuryModal hidden';
      ...
      modal.innerHTML = `<div class="bbHostJuryCard">
        <button type="button" class="bbHostJuryClose" onclick="bbHostPlayerCloseJury()">×</button>
        <h2 class="bbHostJuryTitle">⚖️ Jury / overrule</h2>
        <p class="bbHostJuryHelp">Corrigeer alleen als de AI/jury een antwoord verkeerd heeft beoordeeld.</p>
        <div class="bbHostJuryList">${rows || '<div class="bbHostJuryEmpty">Nog geen antwoorden.</div>'}</div>
      </div>`;
      modal.classList.remove('hidden');
