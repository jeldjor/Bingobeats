...
function logout(){["spotify_access_token","spotify_refresh_token","spotify_expires_at","spotify_code_verifier","bb_spotify_preflight_ok_at"].forEach(k=>localStorage.removeItem(k));accessToken=refreshToken="";expiresAt=0;updateStatus()}window.onSpotifyWebPlaybackSDKReady=()=>{};
...
(function(){
  ...
  function spotifyTrackToBB(track){
    if(!track || !track.id || track.is_local) return null;
    return {
      id: track.id,
      uri: track.uri || ('spotify:track:' + track.id),
      name: track.name || 'Onbekend',
      artists: (track.artists || []).map(a => a.name).filter(Boolean).join(', ') || 'Onbekend',
  ...
  async function spotifyGetJson(url){
    const token = await getToken();
    if(!token) throw new Error('Niet ingelogd bij Spotify.');
    const r = await fetch(url, { headers:{ Authorization:'Bearer ' + token, Accept:'application/json' } });
    const d = await r.json().catch(()=>({}));
    if(!r.ok){
      const msg = d?.error?.message || d?.error_description || r.statusText || 'Spotify fout';
  ...
  async function fetchAllSpotifyPages(url){
    ...
    let next = url;
    while(next){
      const data = await spotifyGetJson(next);
      all.push(...(data.items || []));
      next = data.next || '';
    }
    return all;
  ...
  async function bbImportSelectedPlaylist(){
    try{
      ...
      // Haal tracks op via de playlist-items endpoint. Geen market=from_token gebruiken:
      // sommige Spotify accounts/playlists geven daarop onterecht 403 Forbidden.
      const cleanId = encodeURIComponent(playlistId);
      const primaryUrl = `https://api.spotify.com/v1/playlists/${cleanId}/tracks?limit=100&additional_types=track`;
      const fallbackUrl = (playlist.tracks?.href || `https://api.spotify.com/v1/playlists/${cleanId}/tracks`).split('?')[0] + '?limit=100&additional_types=track';

      let items;
      try{
        items = await fetchAllSpotifyPages(primaryUrl);
      }catch(firstError){
        if(fallbackUrl === primaryUrl) throw firstError;
        items = await fetchAllSpotifyPages(fallbackUrl);
      }
      const seen = new Set();
      const imported = [];
      for(const item of items){
        const t = spotifyTrackToBB(item.track);
        if(!t || seen.has(t.id)) continue;
        seen.add(t.id);
        imported.push(t);
      }
      if(!imported.length){
        setPlaylistStatus('Geen bruikbare Spotify-tracks gevonden in deze playlist.', false);
        return;
      }
      ...
      let help = 'Playlist importeren mislukt: ' + msg;
      if(/scope|permission|forbidden|insufficient|rechten|403/i.test(msg)){
        help += ' — Log uit bij Spotify en log opnieuw in, zodat playlist-read-private en playlist-read-collaborative worden toegestaan.';
      }
      setPlaylistStatus(help, false);
    }
  }
